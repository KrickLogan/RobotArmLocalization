...
image_fnames = cu.get_image_fnames(mask_fnames)

image_fnames = cu.remove_bad_img_files(image_fnames)

all_image_fnames = list(sorted(os.listdir("Data/PNGImages")))

image_fnames, mask_fnames = cu.remove_unmatched_fnames(image_fnames, mask_fnames, all_image_fnames)
...
for mask in mask_fnames:
    np_masks.append((np.array(Image.open(os.path.join("Data/PNGMasks", mask)).convert('L')), mask))
...
# rework_masks should never have an issue, as we now convert the masks to grayscale and strip alpha layer when loading the images
