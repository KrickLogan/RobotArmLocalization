...
def main():
    ...
    print (f"label: {claw.label} Score: {claw.score} Box: {claw.box} Mask: {claw.mask} Average depth: {claw.get_depth(depth_arr)}")

    # The following prints the details and average depth of base that is given by model after detection
    base = detector.get_base()
    print (f"label: {base.label} Score: {base.score} Box: {base.box} Mask: {base.mask} Average depth: {base.get_depth(depth_arr)}")
    ...
    print (f"label: {obj.label} Score: {obj.score} Box: {obj.box} Mask: {obj.mask} Average depth: {obj.get_depth(depth_arr)}")
